...
@group(2) @binding(0) var store_depht: texture_storage_2d<r32uint, write>;

@vertex fn vertex(@location(0) vertex: vec2<f32>, tile: Tile) -> VertexOutput
{
    return VertexOutput
    (
        camera.view_proj * matrix * vec4<f32>(vertex.x + tile.pos.x, vertex.y + tile.pos.y, 0., 1.),
        tile.uv
    )
}
...
{
    textureStore(store_depht, in.tex_coords,vec4<u32>(in.layer, 0u, 0u, 1u))
