...
use egui::epaint::{ahash::HashMap, emath::NumExt, PaintCallbackInfo, Primitive, Vertex};
...
impl Renderer {
    ...
    ) -> Self 
    {
        if msaa_samples == 0 
        {
            panic!("msaa sample count has to be at least 1 \n" )
        }
    ...
    {
        let pixels_per_point = screen_data.scale;
        let size_in_pixels = [screen_data.width, screen_data.height];
                ...
                match &clipped_primitive.primitive {
                    ...
                    Primitive::Callback(callback) => {
                        if let Some(c) = callback.callback.downcast_ref::<Callback>() {
                            ...
                            panic!("Unknown paint callback: expected `egui_wgpu::Callback`");
                        };
                        acc
                    }
                }
